  ...
  context '#perform' do
    ...
    describe 'company code' do
      it 'returns Company was not found. if no company code is provided' do
        message = Message.new('0123456789', 'a 18:00')
    ...
    describe 'reservation' do
      ...
      it 'returns time slot not available if invalid time is provided' do
        ...
        expect(message.perform).to eql("#{company.name}: you can not make a reservation at this time. Next 3 available spot(s) are: 09:00, 09:15, 09:30.")
      end
      ...
      it 'returns time slot not available if time slot is occupied' do
        ...
        message = Message.new('0123456789', "#{company.code} 09:00")
        expect(message.perform).to eql("Reservation created for #{company.name} at 09:00 AM.")

        message = Message.new('0123456788', "#{company.code} 09:00")
        expect(message.perform).to eql("#{company.name}: you can not make a reservation at this time. Next 3 available spot(s) are: 09:15, 09:30, 09:45.")
      end
    end
  end
end
